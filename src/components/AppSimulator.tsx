...
function TabItem({ icon, label, active, onClick }: { icon: React.ReactElement; label: string; active: boolean; onClick: () => void }) {
    return (
        <button onClick={onClick} className={`flex flex-col items-center gap-1 transition-all duration-300 relative group ${active ? 'text-green-500 scale-105' : 'text-slate-300 hover:text-slate-400'}`}>
            ...
            <div className="w-6 h-6 flex items-center justify-center">{React.cloneElement(icon, { strokeWidth: active ? 2.5 : 2 } as React.SVGAttributes<SVGElement>)}</div>
...
export default function MoodiesFinal() {
    ...
    useEffect(() => {
        if (initialData.length > 0) setEntries(initialData);
    }, [initialData]);
    ...
    const handleSave = () => {
        ...
        const newEntry: Entry = {
            ...
            title: title || sim.placeholder_title,
            note: note,
            date: sim.time_today,
    ...
    return (
        <section className="py-12 bg-zinc-100 min-h-screen flex flex-col items-center justify-center font-sans gap-8 overflow-x-hidden">
            ...
            {/* SİMÜLATÖR — responsive scale wrapper */}
            <div className="flex items-start justify-center w-full" style={{ minHeight: "812px" }}>
                <style>{`
                    .sim-scaler { transform-origin: top center; }
                    @media (max-width: 430px)  { .sim-scaler { transform: scale(0.82); margin-bottom: -145px; } }
                    @media (max-width: 390px)  { .sim-scaler { transform: scale(0.76); margin-bottom: -195px; } }
                    @media (max-width: 360px)  { .sim-scaler { transform: scale(0.70); margin-bottom: -244px; } }
                `}</style>
                <div className="sim-scaler">
                                    ...
                                    <motion.div key="stats" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-5 pt-2">
                                        ...
                                        <div className="bg-slate-100 p-1 rounded-[14px] flex mb-6">
                                            {(['week', 'month', 'all'] as const).map(periodKey => (
                                                <button key={periodKey} onClick={() => setStatPeriod(periodKey)}
                                        ...
                                        <div className="grid grid-cols-2 gap-4 mb-6">
                                            ...
                                            <div className="bg-white p-5 rounded-[24px] border border-slate-50 shadow-sm flex flex-col justify-between h-28">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div className="w-9 h-9 rounded-full bg-slate-100 flex items-center justify-center text-xl">{moodConfig[mostFrequentMood].emoji}</div>
                                                    <span className="text-lg font-bold text-slate-900">{sim[moodConfig[mostFrequentMood].dictKey as keyof typeof sim]}</span>
                                                </div>
                                            </div>
                                        </div>
                                                    ...
                                                    return (
                                                        <div key={mood} className="flex flex-col items-center justify-end gap-3 w-full h-full group cursor-pointer relative">
                                                            <div className="w-full relative flex items-end justify-center rounded-t-[10px] transition-all duration-500 hover:brightness-90"
                                                                style={{ height: count > 0 ? `${heightPercent}%` : '4px', backgroundColor: count > 0 ? cfg.color : '#F3F4F6', minHeight: count > 0 ? '20px' : '4px' }}>
                                                                {count > 0 && (
